...
Aplicação web em Streamlit para avaliação de risco ambiental em postos de combustíveis.
"""
...
def build_interactive_map(df_points, boundary_wgs, hex_grid_wgs, risk_col, original_boundary_crs):
    ...
    def style_hex(feature):
        risco = feature["properties"].get("risco_medio", 0)
        return {"fillColor": colormap(risco), "color": "black", "weight": 0.5, "fillOpacity": 0.55}
    ...
    for _, row in df_points.iterrows():
        ...
        for c, label in [
            ("dist_poco_mais_prox_m", "Dist. poço mais próximo (m)"),
            ("pocos_500m", "Poços em 500 m"),
            ("pocos_1km", "Poços em 1 km"),
            ("pocos_5km", "Poços em 5 km"),
        ]:
            if c in row.index:
                popup_lines.append(f"<b>{label}:</b> {row[c]}")
...
def make_static_maps(df_points_wgs, boundary_wgs, hex_grid_wgs, original_boundary_crs):
    ...
    if len(hexes):
        hexes.plot(column="risco_medio", legend=True, ax=ax, cmap="viridis", edgecolor="black", linewidth=0.3, alpha=0.7)
    postos.plot(ax=ax, markersize=10, color="red")
    ax.set_title("Mapa hexagonal de risco médio — Goiás", fontsize=14)
    ...
    ax.add_artist(AnchoredSizeBar(
        ...
        color="black", frameon=True, size_vertical=0.01, fontproperties=fm.FontProperties(size=9)
    ))
    ax.text(0.01, 0.01,
    ...
    fig.tight_layout()
    out_risco = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
    fig.savefig(out_risco.name, dpi=220)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(10, 10))
    goias.boundary.plot(ax=ax, linewidth=1.8, color="black")
    if len(hexes):
        hexes.plot(column="hotspot", legend=True, ax=ax, cmap="Reds", edgecolor="black", linewidth=0.3, alpha=0.6)
    postos.plot(ax=ax, markersize=10, color="blue")
    ax.set_title("Hotspots de risco ambiental — Goiás", fontsize=14)
    ax.set_xlabel("Longitude (graus)")
    ax.set_ylabel("Latitude (graus)")
    ax.grid(True, linestyle="--", linewidth=0.4, alpha=0.4)
    add_north_arrow(ax)
    ...
    ax.text(0.01, 0.01,
            ...
            bbox=dict(facecolor="white", alpha=0.8, edgecolor="gray"))
    fig.tight_layout()
    out_hotspots = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
    fig.savefig(out_hotspots.name, dpi=220)
    plt.close(fig)
...
st.sidebar.caption("""
O modelo de previsão de risco é um arquivo treinado com inteligência artificial.
Ele calcula a probabilidade de contaminação ambiental com base nos dados do posto.
""")
...
required_basic = ["UTM_E_m", "UTM_N_m", "ZONA"]
missing_basic = [c for c in required_basic if c not in df.columns]
if missing_basic:
    st.error(f"O CSV precisa conter: {required_basic}. Faltando: {missing_basic}")
    st.stop()
...
if run_button:
    ...
    try:
        ...
        st.error(f"Erro ao gerar a grade hexagonal: {e}")
        st.stop()

    try:
        fmap = build_interactive_map(df_points=df_scored, boundary_wgs=goias_wgs, hex_grid_wgs=hex_grid_wgs,
                                     risk_col=risk_col, original_boundary_crs=boundary_original_crs)
    ...
    try:
        ...
        st.error(f"Erro ao gerar os mapas estáticos: {e}")
        st.stop()
...
if st.session_state.analysis_done:
    ...
    with open(png_risco, "rb") as f:
        st.download_button("⬇️ Baixar PNG (risco médio)", data=f.read(),
                           file_name="mapa_hex_risco_goias.png", mime="image/png")
    with open(png_hotspots, "rb") as f:
        st.download_button("⬇️ Baixar PNG (hotspots)", data=f.read(),
                           file_name="mapa_hex_hotspots_goias.png", mime="image/png")
